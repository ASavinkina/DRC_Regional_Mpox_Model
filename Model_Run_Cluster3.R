...
# Call in model code:
source("Stochastic_Model_Code_3.24.25_ShortTerm.R")
...
out <- parLapply(cl = cl, X = 1:n, fun = function(x) {
  ...
  set.seed(12345 + x)
  ssa(y_init, reactions, t(nu), params, tf = 365, method = ssa.btl(f = 10))$data
})
...
write.csv(output_all, file="Full_output_test_32425_st.csv")